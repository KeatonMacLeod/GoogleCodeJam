...
public class RevengeOfThePanckaes {
	public static void main (String[] args)
	{
		try
		{
			Scanner in = new Scanner(new BufferedReader(new FileReader("revengeofthepancakeslarge.txt")));
	...
	{
		int numberOfFlips = 0;
		boolean haventSeenSad = true;
		boolean lastSeenHappy = true;
		
		for (int a = pancakes.length()-1; a >= 0; a--)
		{
			if (a == pancakes.length()-1 && pancakes.charAt(a) == '-' && lastSeenHappy)
			{
				haventSeenSad = false;
				lastSeenHappy = false;
				numberOfFlips++;
			...
			else if (a == pancakes.length()-1 && pancakes.charAt(a) == '+' && !lastSeenHappy)
			{
				lastSeenHappy = true;
			}
